...
<?php
    ...
    <?php
		...
		$query_edit_story = $connect->prepare("select stories.title,
														stories.story
														from stories
														where story_id=?");
		if(!$query_edit_story){
		...
		$query_edit_story->bind_param("i", $story_id);
		$query_edit_story->execute();
        $query_edit_story->bind_result($temp_title, $temp_story);
            //$title="";
            //$story="";
		while($query_edit_story->fetch()){
                   $title= $temp_title;
				   $story= $temp_story;
                }
		
		//close query
		$query_edit_story->close();   
	
	?>
	<h1> Edit your story </h1>
			<form action="edit_operation.php" method="POST">
			<p>Story Title: <input type="text" name="story_title" value="<?php echo $title; ?>" required></p>
